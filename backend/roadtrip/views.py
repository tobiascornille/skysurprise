...
class RoadtripData:
    ...
    def plan_trip(self):
        ...
        obj_next_day_in_planning = date_string_to_object(calculate_new_date(day_in_planning))

        # add as much extra cities as possible within the flightbudget,
        # keeping in mind that we also have to get home in time
        while flightbudget - self.get_flightprice(lastLocation, self.originplace, self.inbounddate) > 0 and obj_next_day_in_planning <= obj_inbounddate:
            add_city(last_city, day_in_planning, days_per_city)
            last_city = self.cities[-1]

            day_in_planning = calculate_new_date(day_in_planning, days_per_city)
            obj_day_in_planning = date_string_to_object(day_in_planning)
